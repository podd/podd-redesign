...
public class SimpleUUIDPurlProcessorFactoryTest extends AbstractPoddRdfProcessorFactoryTest<PoddPurlProcessor>
{
    ...
    private final String[] temporaryUriArray = new String[] { "urn:temp", "urn:temporary" };
    
    @Override
    protected PoddRdfProcessorFactory<PoddPurlProcessor> getNewPoddRdfProcessorFactory()
    {
        final SimpleUUIDPurlProcessorFactory factory = new SimpleUUIDPurlProcessorFactory();
        factory.setTemporaryUriArray(this.temporaryUriArray);
        return factory;
    }
    ...
    public void testSPARQLQueryViaExecution() throws Exception
    {
        ...
        try
        {
            ...
            while(result.hasNext())
            {
                ...
                for(final String tempPrefix : this.temporaryUriArray)
                {
                    if(statement.getSubject().stringValue().startsWith(tempPrefix)
                            || statement.getObject().stringValue().startsWith(tempPrefix))
                    {
                        tempUriFound = true;
                        break;
                    }
