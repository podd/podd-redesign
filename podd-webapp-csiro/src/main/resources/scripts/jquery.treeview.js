...
;(function($) {
    ...
    function rememberClicked(context, clicked) {
        var newTree = true;
        if (null != context.baseURI) {
         newTree = context.baseURI.indexOf(clicked) != -1
        }

        var data = [];
        var prev_clicked = $.cookie('podd_treeview');
        if (!newTree && prev_clicked) {
            data = prev_clicked.split(", ");
        }
        var index = $.inArray(clicked, data);
        if (index > -1) {
            data = $.map(data, function(n, i) {
                return (n == clicked || n == "") ? null : n;
            });
        } else {
            data[data.length] = clicked;
        }
        $.cookie('podd_treeview', data.join(", "));
    }
	...
	$.extend($.fn, {
		...
		heightToggle: function(animated, callback) {
            //rememberClicked(this.context, this.prevObject[0].id);
            animated ?
				this.animate({ height: "toggle" }, animated, callback) :
				this.each(function(){
					jQuery(this)[ (jQuery(this).css("visibility") == "hidden" || jQuery(this).css("display")) == "none" ? "show" : "hide" ]();
					if(callback)
